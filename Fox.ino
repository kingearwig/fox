...
int pause = 660;  // pause between transmissions, 22 = 1 second (approx)
...
char beaconstring[] = "CQ NI9CA/FOX CQ NI9CA/FOX"; // change to your call sign
char idstring[] = "DE NI9CA"; // change to your call sign
int randNumber;
...
for (int x = 0; x < pause; x ++)  // while waiting between transmissions, look for DTMF
{
  ...
  if (thischar) {                             // decide what to do if DTMF tone is received
    Serial.print("thischar=");Serial.println(thischar);
    switch (thischar) {
